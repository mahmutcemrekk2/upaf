...
// Custom Vite middleware to act as the CORS proxy for API tests and Token generation in Dev mode
const authProxyPlugin = () => ({
  name: 'auth-proxy-middleware',
  configureServer(server) {
    server.middlewares.use('/api/step-definitions', async (req, res, next) => {
      if (req.method !== 'GET') {
        res.statusCode = 405;
      ...
      try {
        ...
        if (!fs.existsSync(filePath)) {
          ...
          res.end(JSON.stringify({ error: 'CommonSteps.java not found' }));
          return;
        }
    ...
    server.middlewares.use('/proxy', (req, res, next) => {
      ...
      req.on('end', async () => {
        try {
          ...
          const { url, method, headers, body: requestBody, extractCookies } = parsed;

          console.log('\n--- UPAF VITE PROXY ROUTING ---');
          ...
          console.log(`[Vite Proxy] Response Status: ${status}`);
          ...
          console.error(`[Vite Proxy Critical Error]`, error);
          res.statusCode = 500;
          res.setHeader('Content-Type', 'application/json');
          res.setHeader('Access-Control-Allow-Origin', '*');
          res.end(JSON.stringify({ error: error.message }));
